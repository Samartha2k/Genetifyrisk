...
// Height/Weight unit toggle functions
...
function toggleWeightUnit() {
  ...
  document.getElementById('weightKg').style.display = unit === 'kg' ? 'block' : 'none';
  document.getElementById('weightLbs').style.display = unit === 'lbs' ? 'block' : 'none';
  calculateBMI();
}
...
function calculateBMI() {
  ...
  if (heightUnit === 'cm') {
    heightCm = parseFloat(document.getElementById('heightCmValue').value) || 0;
  } else {
  ...
  if (weightUnit === 'kg') {
    weightKg = parseFloat(document.getElementById('weightKgValue').value) || 0;
  } else {
    const lbs = parseFloat(document.getElementById('weightLbsValue').value) || 0;
...
function renderDiseases(category = 'all') {
  ...
  filtered.forEach(disease => {
    ...
    card.setAttribute('aria-pressed', selectedDisease && selectedDisease.id === disease.id ? 'true' : 'false');
...
function selectDisease(disease) {
  ...
  showToast(`Selected: ${disease.name}`);
  hideError();
}
  ...
  container.innerHTML = familyMembers.map((member, index) => `
    <div class="family-member">
      <div class="family-member-header">
        ...
        <button type="button" class="remove-btn" onclick="removeFamilyMember(${member.id})" aria-label="Remove family member ${index + 1}">Remove</button>
      </div>
      <div class="form-row">
        <div class="form-group">
          <label>Relationship</label>
          <select onchange="updateFamilyMember(${member.id}, 'relation', this.value)">
            <option value="" ${!member.relation ? 'selected' : ''}>Select...</option>
            ...
            <option value="grandparent" ${member.relation === 'grandparent' ? 'selected' : ''}>Grandparent</option>
            <option value="aunt-uncle" ${member.relation === 'aunt-uncle' ? 'selected' : ''}>Aunt/Uncle</option>
        ...
        <div class="form-group">
          ...
          <select onchange="updateFamilyMember(${member.id}, 'status', this.value)">
            <option value="" ${!member.status ? 'selected' : ''}>Select...</option>
            <option value="affected" ${member.status === 'affected' ? 'selected' : ''}>Affected</option>
            <option value="carrier" ${member.status === 'carrier' ? 'selected' : ''}>Carrier</option>
          </select>
        </div>
      </div>
      <div class="form-group">
        <label>Age of Onset <span class="optional">(Optional)</span></label>
        <input type="number" placeholder="Age when diagnosed" value="${member.ageOnset || ''}" onchange="updateFamilyMember(${member.id}, 'ageOnset', this.value)">
      </div>
    </div>
  `).join('');
}
...
// Navigation to specific step (for clicking on progress steps)
function goToStep(step) {
  // Can only go to completed steps or current step
  if (step > currentStep) return;
  if (step === 5) return; // Can't click to results
...
function updateStepDisplay() {
  ...
  if (btnText) {
    btnText.textContent = currentStep === 4 ? 'Calculate Risk' : 'Next';
  } else {
    nextBtn.textContent = currentStep === 4 ? 'Calculate Risk' : 'Next';
  }
}
...
function updateProgress() {
  ...
  steps.forEach((step, index) => {
    ...
    if (stepNum < currentStep) {
      ...
      step.disabled = false; // Enable completed steps for navigation
    } else if (stepNum === currentStep) {
      step.classList.add('active');
      step.setAttribute('aria-current', 'step');
      step.disabled = false;
    } else {
      step.disabled = true; // Disable future steps
    }
  });
...
function validateStep(step) {
  ...
  switch(step) {
    case 1:
      if (!selectedDisease) {
        errors.push('Please select a disease to assess');
      }
      break;
    ...
    case 2:
      ...
      if (!age) {
        ...
        document.getElementById('age').parentElement.classList.add('has-error');
      ...
      if (!sex) {
        ...
        document.getElementById('sex').parentElement.classList.add('has-error');
      }

      if (!ethnicity) {
        errors.push('Ethnicity/Ancestry is required');
        document.getElementById('ethnicity').parentElement.classList.add('has-error');
      }
      break;

    case 3:
      // Optional validation for family history
      if (document.getElementById('hasAffectedFamily').value === 'yes' && familyMembers.length > 0) {
        familyMembers.forEach((member, index) => {
          if (!member.relation || !member.status) {
            errors.push(`Family Member ${index + 1}: Please select relationship and status`);
          }
        });
      }
      break;
  ...
  if (errors.length > 0) {
    showError('Please correct the following errors:', errors);
    return false;
  }

  // Clear any error states on form fields
...
function calculateRisk() {
  ...
  // Small delay to show loading state (simulates calculation)
  setTimeout(() => {
    collectFormData();
...
function collectFormData() {
  formData = {
    ...
    bmi: parseFloat(calculateBMI()) || null,
...
function exportToPDF() {
  try {
    ...
    btn.innerHTML = '<svg class="spinner" width="20" height="20" viewBox="0 0 24 24"><circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="3" fill="none" stroke-dasharray="31.4 31.4" stroke-linecap="round"><animateTransform attributeName="transform" type="rotate" from="0 12 12" to="360 12 12" dur="1s" repeatCount="indefinite"/></circle></svg> Generating...';
    btn.disabled = true;
...
function generatePDFContent(doc) {
  ...
  const drawSectionHeader = (title, y) => {
    ...
    const fontSize = title.length > 20 ? 12 : 14;
  ...
  const drawInfoBox = (label, value, y, color = [13, 148, 136]) => {
    doc.setFillColor(248, 250, 252);
  ...
  yPos = drawInfoBox('Disease:', disease.name, yPos, [16, 185, 129]);
  yPos = drawInfoBox('Inheritance Type:', categoryLabel, yPos, [16, 185, 129]);
  yPos = drawInfoBox('Population Prevalence:', populationRisk, yPos, [16, 185, 129]);
...
// Performance optimization: Pause DNA animation when page not visible
document.addEventListener('visibilitychange', () => {
  const dnaHelix = document.querySelector('.dna-helix');
  if (dnaHelix) {
    dnaHelix.style.animationPlayState = document.hidden ? 'paused' : 'running';
  }
});
